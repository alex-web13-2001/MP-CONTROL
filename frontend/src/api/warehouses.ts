...
export async function downloadCrossExcel(params: {
  shop_id: number
  period?: number
  marketplace: 'wildberries' | 'ozon'
}): Promise<void> {
  const endpoint = params.marketplace === 'wildberries'
    ? '/warehouses/wb/cross/excel'
    : '/warehouses/ozon/cross/excel'
  const response = await apiClient.get(endpoint, {
    params: { shop_id: params.shop_id, period: params.period ?? 30 },
    responseType: 'blob',
  })
  ...
  link.href = url
  const mp = params.marketplace === 'wildberries' ? 'wb' : 'ozon'
  link.setAttribute('download', `cross_logistics_${mp}_${params.shop_id}_${params.period ?? 30}d.xlsx`)
  document.body.appendChild(link)
  link.click()
  link.remove()
  window.URL.revokeObjectURL(url)
...
export async function getOzonCrossAIAnalysis(params: {
  ...
  // but keeps fetch alive for longer, matching the pattern used in campaign AI.
  ...
  try {
    const response = await fetch(`${baseUrl}/warehouses/ozon/cross/ai-analysis?${qs.toString()}`, {
      method: 'POST',
      headers: {
...
// WB Cross AI Analysis (same type as Ozon — same JSON structure)
export type WbCrossAIAnalysis = OzonCrossAIAnalysis

export async function getWbCrossAIAnalysis(params: {
  shop_id: number
  period?: number
  force?: boolean
}): Promise<WbCrossAIAnalysis> {
  const { useAuthStore } = await import('@/stores/authStore')
  const token = useAuthStore.getState().token
  const baseUrl = import.meta.env.VITE_API_URL || '/api/v1'
  ...
  try {
    const response = await fetch(`${baseUrl}/warehouses/wb/cross/ai-analysis?${qs.toString()}`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
