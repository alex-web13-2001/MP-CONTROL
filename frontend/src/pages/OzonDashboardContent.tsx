...
function fmt(v: number) { return (v ?? 0).toLocaleString('ru-RU', { maximumFractionDigits: 0 }) + ' ₽' }
function fmtN(v: number) { return (v ?? 0).toLocaleString('ru-RU') }
function fmtPct(v: number) { return `${(v ?? 0).toFixed(1)}%` }
...
function DeltaBadge({ value, invert }: { value: number; invert?: boolean }) {
  const v = value ?? 0
  if (v === 0) return <span className="text-xs text-[hsl(var(--muted-foreground)/0.5)]">—</span>
...
function PPDelta({ value, invert, className = '' }: { value: number; invert?: boolean; className?: string }) {
  const v = value ?? 0
  if (v === 0) return <span className={`text-[11px] text-[hsl(var(--muted-foreground)/0.4)] ${className}`}>—</span>
  ...
  return (
    <span className={`inline-flex items-center gap-0.5 text-xs font-semibold ${good ? 'text-emerald-400' : 'text-red-400'} ${className}`}>
      ...
      {v > 0 ? '+' : ''}{v.toFixed(1)}<span className="text-[10px] font-normal opacity-70">п.п.</span>
    </span>
  )
}
