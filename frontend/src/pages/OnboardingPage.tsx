...
/* ────────────────── Types ────────────────── */
type Marketplace = 'wildberries' | 'ozon' | null
  ...
  return (
    <div className="space-y-6 text-center">
      ...
      <p className="text-[hsl(var(--muted-foreground))]">
        Подключите ваш первый магазин для начала работы
      </p>
...
function StepSyncing({
  ...
  return (
    <div className="max-w-lg mx-auto space-y-6 text-center py-8">
      ...
      {/* Dashboard button — only when done */}
      {(isDone || isError) && (
        <motion.button
...
export default function OnboardingPage() {
  ...
  // Return-visit: if a shop is already syncing, jump straight to progress
  useEffect(() => {
    const syncingShop = shops.find((s) => s.status === 'syncing')
    if (syncingShop) {
      setCreatedShopId(syncingShop.id)
      setMarketplace(syncingShop.marketplace as Marketplace)
      setFormData({ apiKey: '', shopName: syncingShop.name })
      setStep(3)
    }
  ...
  const handleDashboard = async () => {
    // Refresh shops from API so authStore gets updated status (active, not syncing)
    try {
      const shopsResp = await apiClient.get('/shops')
      setShops(shopsResp.data)
    } catch {
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-6 md:p-8 shadow-xl">
          <AnimatePresence mode="wait">
            ...
            >
              {step === 0 && (
                <StepMarketplace onSelect={handleMarketplaceSelect} />
              )}
              {step === 1 && (
                <StepApiKeys
                  marketplace={marketplace}
