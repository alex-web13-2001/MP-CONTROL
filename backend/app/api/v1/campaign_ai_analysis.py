...
SYSTEM_PROMPT_WB = """Ты — эксперт по рекламе на Wildberries. Ты анализируешь одну конкретную рекламную кампанию WB и даёшь конкретные рекомендации.

## КАК РАБОТАЕТ РЕКЛАМА НА WILDBERRIES

### Управление кампанией
Продавец может настроить:
- **Ставка CPM** — стоимость за 1000 показов. Задаётся в КОПЕЙКАХ (1₽ = 100 копеек). Но для удобства в анализе указывай ставки в РУБЛЯХ.
- **Бюджет** — общий на кампанию (не дневной, не недельный).
- **Размещение** — Поиск, Каталог/Рекомендации, или оба сразу.
- **Минус-фразы** — исключение нерелевантных поисковых запросов (до 1000 шт).
- **Фиксированные фразы** — закрепление важных запросов.
- **Товары** — добавить/убрать конкретные артикулы из кампании.

### Финансовая модель WB
- **Выручка (revenue)** = цена товара для покупателя (до СПП).
- **СПП** — Скидка Постоянного Покупателя, 0-30%. Продавец НЕ контролирует.
- **Payout** = сумма на руки продавцу. Уже МИНУС комиссия WB, логистика, хранение. НЕ вычитай из payout комиссию повторно!
- **Прибыль до рекламы** = payout − себестоимость

### 3 типа рекламных продаж
WB атрибутирует заказы по модели последнего касания:
- **Прямые** — рекламируемые товары (те, у которых есть показы/клики/расход). Их обычно 1-5.
- **Модельные** — другие товары из той же карточки (imt_id): другой размер, цвет, фасовка
- **Ассоциированные** — товары из ДРУГИХ карточек (кросс-продажа)

ВАЖНО: в данных указано ТОЧНОЕ кол-во рекламируемых товаров. Не путай с общим числом SKU в кампании!

### События: стокауты
Событие «Нет остатков» может означать отсутствие на КОНКРЕТНОМ региональном складе, а не глобальное отсутствие товара. Не утверждай, что «товара нет на складе глобально», если нет прямых данных об этом. Говори аккуратно: «зафиксировано N событий нехватки остатков».

## КАК АНАЛИЗИРОВАТЬ

### Юнит-экономика — ОФОРМЛЯЙ КАК ТАБЛИЦУ!
Для КАЖДОГО прямого рекламного товара создай markdown-таблицу:

| Параметр | Сумма (₽) |
|---|---|
| Выручка (ср. розн. цена) | X |
| Комиссия WB (вкл. СПП) | -X |
| Payout (на руки) | X |
| Себестоимость | -X |
| **Прибыль до рекламы** | **X** |
| Допустимый CAC | X |
| Реальный CAC | X |

Затем ОБЩИЙ P&L кампании — тоже как таблица:

| Тип продаж | Заказов | Выручка | Ест. прибыль* |
|---|---|---|---|
| Прямые | N | X₽ | X₽ |
| Модельные | N | X₽ | ≈X₽ |
| Ассоциированные | N | X₽ | — |
...
*Оценочная прибыль модельных = средняя прибыль прямого товара × кол-во модельных заказов (т.к. товар из той же карточки, маржа аналогична). Если данных нет — ставь ≈ или «нет данных».

### Эффективность рекламы
- CTR = клики / показы × 100%
- CPC = расход / клики (в рублях)
- CR = заказы / клики × 100%
- DRR = расход / рекл.выручка × 100% — чем ниже, тем лучше. Но конкретного «порога» нет — оценивай относительно маржи товара
- CPM фактический = расход / (показы / 1000) — в рублях

### Рекомендации
Рекомендуй ТОЛЬКО реально доступные действия:
- Ставку CPM (укажи в рублях, например «12₽ CPM»)
- Бюджет (в ₽)
- Добавить/убрать товары из кампании
- Минус-фразы (если есть данные по фразам)
- Размещение (Поиск/Каталог/Рекомендации)
- Пополнить остатки

## ФОРМАТ ОТВЕТА

Верни СТРОГО JSON-массив. Без обёрток ```json``` — только чистый JSON.
Не добавляй секций, которых нет в шаблоне ниже.

[
  {
    "id": "verdict",
    "title": "🎯 Вердикт",
    "content": "Название кампании «XXX». 2-3 предложения. Прибыльна/убыточна с конкретной цифрой P&L. Главная проблема или сила.",
    "type": "verdict",
    "status": "negative|neutral|positive"
  },
  {
    "id": "unit_economics",
    "title": "💰 Юнит-экономика и P&L",
    "content": "Markdown-таблицы: 1) юнит-экономика per-unit для каждого прямого товара, 2) общий P&L кампании с разбивкой по типам продаж (прямые/модельные/ассоциированные). Чистая прибыль = прибыль от всех заказов − расход на рекламу.",
    "type": "section"
  },
  {
    "id": "sales_breakdown",
    "title": "📦 Структура продаж",
    "content": "Прямые: N шт, X₽. Модельные: N шт, X₽. Ассоциированные: N шт, X₽. Вывод о работе карточки.",
    "type": "section"
  },
  {
    "id": "ad_efficiency",
    "title": "📊 Эффективность рекламы",
    "content": "CPM, CTR, CPC, CR, DRR — средние и тренды. Лучшие/худшие дни.",
    "type": "section"
  },
  {
    "id": "events",
    "title": "⚡ События",
    "content": "Ключевые: дата → что изменилось → как повлияло. Стокауты — уточни: региональные или глобальные.",
    "type": "section"
  },
  {
    "id": "strategy",
    "title": "💡 Рекомендации",
    "content": "Конкретные действия с цифрами.",
    "type": "strategy",
    "actions": [
      {"action": "Ставка CPM", "value": "X₽", "priority": "high|medium|low"},
      {"action": "Бюджет", "value": "X₽", "priority": "high|medium|low"}
    ]
  }
]
...
- Пиши на русском языке.
- В вердикте указывай НАЗВАНИЕ кампании (из данных), не только ID.
- Юнит-экономику и P&L оформляй ТАБЛИЦАМИ (markdown), не текстом.
- Все ставки и CPM указывай в РУБЛЯХ (не в копейках).
- Каждая секция = 3-7 предложений. Не лей воду.
- Не выдумывай данные, которых нет.
- Рекламируется столько товаров, сколько указано — не больше!
- Не утверждай «товар отсутствовал на складе», если это могут быть региональные стоки.
- Если данных по ключевым фразам нет — не упоминай фразы.
- Если событий нет — не включай секцию events.
"""
...
async def analyze_campaign_ai(
    ...
    try:
        ...
        # ── 1b. Get campaign name ──
        campaign_name = ""
        try:
            cn_rows = ch.query(
                "SELECT name FROM mms_analytics.dim_advert_campaigns WHERE advert_id = {cid:UInt64}",
                parameters={"cid": campaign_id}
            ).result_rows
            if cn_rows and cn_rows[0][0]:
                campaign_name = cn_rows[0][0]
        except Exception:
            pass
        ...
        if mp == "ozon":
            ...
            # WB: skip prod_rows — we use only campaign-attributed data from fact_advert_stats_v3
            # fact_orders_raw contains ALL orders for these SKUs (organic+ad), which confuses AI
            prod_rows = []
...
"""
    ...
    if mp == "ozon":
        ...
        # WB: events only for DIRECT SKUs (with bids/views/spend), not model/associated
        direct_skus_for_events = locals().get('direct_skus', set())
        event_nm_ids = list(direct_skus_for_events) if direct_skus_for_events else campaign_skus
    ...
    if mp == "wb":
        stats_header = "Дата | Показы | Клики | CTR% | Рекл.заказы | Корзины | Рекл.выручка | Расход | CPC | CR%"
        stats_sep = "---|---|---|---|---|---|---|---|---|---"
    else:
...
"""
    ...
    if mp == "wb":
        # WB: clean message without prod_orders — only campaign-attributed data
        phrases_section = ""
        if phrases_lines:
            phrases_section = f"""\n### КЛЮЧЕВЫЕ ФРАЗЫ ({len(phrases_lines)} шт.):\n{chr(10).join(phrases_lines)}"""

        events_section = ""
        if events_lines:
            events_section = f"""\n### СОБЫТИЯ ({len(events_raw)} шт.):\n{chr(10).join(events_lines)}"""

        # Count DIRECT products (with views/clicks/spend) — that's what's actually advertised
        n_direct = len(locals().get('direct_skus', set()) or set())
        n_model = locals().get('model_orders', 0)
        n_assoc = locals().get('assoc_orders', 0)

        # Build product info ONLY for direct SKUs
        direct_skus_set = locals().get('direct_skus', set())
        direct_product_lines = []
        model_product_lines = []
        for pil in product_info_lines:
            # Check if this product line is for a direct SKU
            is_direct = False
            for ds in direct_skus_set:
                if str(ds) in pil:
                    is_direct = True
                    break
            if is_direct:
                direct_product_lines.append(pil)
            else:
                model_product_lines.append(pil)

        # CPM in rubles (convert from kopecks logic)
        total_views = sum(ad['views'] for ad in ad_by_date.values())
        cpm_rub = round(total_spend / (total_views / 1000), 2) if total_views > 0 else 0

        campaign_title = campaign_name if campaign_name else f"ID {campaign_id}"

        user_message = f"""Кампания: «{campaign_title}» (ID {campaign_id})
Маркетплейс: Wildberries
Период: {start_date} — {end_date} ({len(all_dates)} дней с данными)

### РЕКЛАМИРУЕМЫЕ ТОВАРЫ ({n_direct} шт. — только те, у которых есть показы/клики/расход):
{chr(10).join(direct_product_lines) if direct_product_lines else 'Нет данных'}

ВАЖНО: В кампании рекламируется {n_direct} товар(ов). Остальные {len(campaign_skus) - n_direct} товаров получили заказы через модельные (та же карточка) или ассоциированные (кросс-продажа) конверсии, НО они НЕ рекламируются напрямую.

### СВОДКА КАМПАНИИ:
- Расход на рекламу: {total_spend:.0f}₽
- Рекламных заказов (все типы): {total_ad_orders}
- Рекламная выручка: {total_ad_rev:.0f}₽
- DRR: {total_drr}%
- Средний CR: {avg_cr}%
- CAC общий (расход / все рекл.заказы): {total_cac}₽
- CPM фактический: {cpm_rub:.0f}₽
{finance_summary}
...
Проанализируй данные кампании. Юнит-экономику и P&L оформи в виде markdown-таблиц (для каждого прямого товара). Ставки указывай в рублях. Оцени прибыльность: сколько прибыли принесли прямые заказы, модельные, ассоциированные. Дай конкретные рекомендации."""
    else:
        # Ozon: keep original format with prod_orders data
...
- Органических: {organic_orders} ({halo_pct}% от общих)
- DRR от рекл.выручки: {total_drr}%
- DRR от ОБЩЕЙ выручки: {total_drr_overall}%
...
Проанализируй ВСЕ данные. Рассчитай полную юнит-экономику. Найди оптимальную стратегию масштабирования с МАКСИМАЛЬНОЙ прибылью! Рекомендации по бюджету — в НЕДЕЛЬНЫХ суммах."""

    # ── 12. Stream from Gemini ──
    system_prompt = SYSTEM_PROMPT_WB if mp == "wb" else SYSTEM_PROMPT
    logger.info("AI analysis mp=%s, prompt_type=%s, user_msg_len=%d, has_previous=%s",
                mp, "WB" if mp == "wb" else "OZON", len(user_message), bool(previous_analysis))
    if mp == "wb":
        logger.info("WB user_message first 500 chars: %s", user_message[:500])
    messages = [
        {"role": "system", "content": [{"type": "text", "text": system_prompt}]},
    ]
    if previous_analysis and mp != "wb":
