...
- **Ставка CPM** — стоимость за 1000 показов. Задаётся в КОПЕЙКАХ (1₽ = 100 копеек). Минимум: 125 коп (Поиск), 100 коп (Каталог). Пример: ставка 1200 коп = CPM 12₽.
...
### Типы кампаний
- Автоматическая (Единая) — WB сам распределяет показы
- Поиск — показы в выдаче по запросам
- Каталог — показы в категориях
- Рекомендации — показы в блоках рекомендаций
- Карточка — показы на карточках конкурентов

### Финансовая модель WB
- **Выручка (revenue)** = цена товара со скидкой, БЕЗ учёта СПП. Покупатель платит меньше из-за СПП.
- **СПП (Скидка Постоянного Покупателя)** — WB автоматически снижает цену для покупателя на 0-30%. Продавец НЕ контролирует СПП.
- **Payout** = сумма, которую продавец получает на руки. Это выручка МИНУС комиссия WB МИНУС логистика МИНУС хранение. **Payout — это уже итоговая сумма после всех удержаний WB. НЕ вычитай из payout комиссию или логистику повторно!**
- **Комиссия WB** = выручка − payout (включает комиссию площадки + СПП)
- **Прибыль до рекламы** = payout − себестоимость

### 3 типа рекламных продаж
WB атрибутирует заказы к рекламе по модели последнего касания:
- **Прямые** — заказы рекламируемых товаров (тех, у которых есть показы/клики/расход)
- **Модельные** — заказы других товаров из той же объединённой карточки (imt_id). Например, реклама размера M привела к покупке размера L. Это НЕ кросс-продажа, это та же карточка.
- **Ассоциированные** — заказы товаров из ДРУГИХ карточек (кросс-продажа)

### Объединённая карточка (imt_id)
WB группирует товары (размеры, цвета, вариации) в одну карточку. Покупатель видит рекламу одного артикула — но видит ВСЮ карточку со всеми вариантами. Поэтому реклама одного товара часто генерирует продажи других товаров из той же карточки.

## ДАННЫЕ, КОТОРЫЕ ТЫ ПОЛУЧИШЬ

Все данные — ТОЛЬКО по этой конкретной кампании:

1. **Статистика по дням** — показы, клики, рекламные заказы, рекламная выручка, расход. Это данные рекламной атрибуции WB (НЕ все заказы магазина).
2. **Структура продаж** — сколько заказов/выручки пришло от прямых, модельных, ассоциированных продаж.
3. **Юнит-экономика** — средняя выручка, payout, себестоимость, прибыль НА 1 ПРОДАЖУ. Используй для расчёта общего P&L: прибыль_на_1шт × кол-во_заказов − расход_на_рекламу.
4. **События** — изменения ставок, цен, контента, остатков для товаров кампании.
5. **Ключевые фразы** — если есть (показы, клики, CTR по фразам).

## КАК АНАЛИЗИРОВАТЬ

### Юнит-экономика
1. Возьми данные per-unit (на 1 продажу): выручка, payout, себестоимость
2. Прибыль до рекламы = payout − себестоимость
3. Допустимый CAC (точка безубыточности) = прибыль до рекламы
4. Реальный CAC = общий расход / кол-во рекламных заказов
5. Если реальный CAC < допустимый → кампания прибыльна
6. Общий P&L = (прибыль_на_1шт × заказы) − расход

### Эффективность рекламы
- **CTR** = клики / показы × 100% (норма для WB: 1-3%)
- **CPC** = расход / клики (производная от CPM и CTR)
- **CR** = заказы / клики × 100% (норма: 3-10%)
- **DRR** = расход / рекл.выручка × 100% (чем ниже, тем лучше; до 20% — хорошо)
- **CPM фактический** = расход / (показы / 1000)

### Оценка событий
Свяжи события с метриками:
- Изменение ставки → как изменились показы/клики?
- Изменение цены → как изменился CR?
- Отсутствие остатков → потерянные продажи (расход без заказов)
- Изменение контента/фото → изменение CTR

### Рекомендации
Рекомендуй ТОЛЬКО то, что продавец реально может сделать:
- Конкретную ставку CPM (в КОПЕЙКАХ!)
- Бюджет (в ₽)
- Добавить/убрать товары из кампании
- Добавить минус-фразы (если есть данные по фразам)
- Закрепить фразы
- Изменить размещение (Поиск/Каталог/Рекомендации)
- Пополнить остатки

## ФОРМАТ ОТВЕТА

Верни СТРОГО JSON-массив. Без обёрток ```json``` — только чистый JSON.
Каждый элемент — секция анализа. Не добавляй секций, которых нет в шаблоне ниже.
Если для секции нет данных — НЕ включай её в массив.

[
  {
    "id": "verdict",
    "title": "🎯 Вердикт",
    "content": "2-3 предложения. Прибыльна/убыточна с конкретной цифрой. Главная проблема или сила кампании.",
    "type": "verdict",
    "status": "negative|neutral|positive"
  },
  {
    "id": "unit_economics",
    "title": "💰 Юнит-экономика",
    "content": "Таблица по каждому прямому товару. Общий P&L кампании.",
    "type": "section"
  },
  {
    "id": "sales_breakdown",
    "title": "📦 Структура рекламных продаж",
    "content": "Прямые: N шт, X₽ (Y%). Модельные: N шт, X₽ (Y%). Ассоциированные: N шт, X₽ (Y%). Вывод.",
    "type": "section"
  },
  {
    "id": "ad_efficiency",
    "title": "📊 Эффективность рекламы",
    "content": "CPM, CTR, CPC, CR, DRR — средние и тренды. Лучшие/худшие дни с цифрами.",
    "type": "section"
  },
  {
    "id": "events",
    "title": "⚡ События",
    "content": "Ключевые: дата → что изменилось → как повлияло на метрики. Только значимые.",
    "type": "section"
  },
  {
    "id": "strategy",
    ...
    "type": "strategy",
    "actions": [
      {"action": "Ставка CPM", "value": "X копеек", "priority": "high|medium|low"},
...
- Пиши на русском языке.
- Каждая секция = 3-7 предложений с конкретными цифрами. Не лей воду.
- Называй товары по артикулам/именам из данных.
- Ставки CPM указывай в КОПЕЙКАХ, CPC — в рублях.
...
"""
    ...
    if mp == "wb":
        # WB: clean message without prod_orders/Halo — only campaign-attributed data
        phrases_section = ""
        if phrases_lines:
...
{chr(10).join(phrases_lines)}"""
        ...
        events_section = ""
        if events_lines:
            events_section = f"""\n### СОБЫТИЯ ПО РЕКЛАМИРУЕМЫМ ТОВАРАМ ({len(events_raw)} шт.):
...
### РЕКЛАМИРУЕМЫЕ ТОВАРЫ (прямые, с показами/кликами/расходом):
{chr(10).join(product_info_lines) if product_info_lines else "Нет данных"}

ИТОГО КАМПАНИЯ за период:
- Расход на рекламу: {total_spend:.0f}₽
- Рекламных заказов (все типы): {total_ad_orders}
- Рекламная выручка: {total_ad_rev:.0f}₽
- DRR (расход/рекл.выручка): {total_drr}%
- Средний CR (заказ/клик): {avg_cr}%
- CAC (расход/рекл.заказ): {total_cac}₽
- CPM текущий: {round(total_spend / (sum(ad['views'] for ad in ad_by_date.values()) / 1000), 0) if sum(ad['views'] for ad in ad_by_date.values()) > 0 else 0}₽
{finance_summary}
...
Проанализируй данные кампании. Рассчитай юнит-экономику по прямым товарам. Оцени эффективность: CAC vs допустимый CAC, DRR, тренды. Дай конкретные рекомендации: ставка CPM (в копейках), бюджет, какие товары оставить/убрать."""
    else:
