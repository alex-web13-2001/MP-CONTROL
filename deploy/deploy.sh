...
#   2. docker compose build + up (backend, celery)
#      → entrypoint.sh автоматически применяет Alembic (PostgreSQL)
#   3. ClickHouse миграции (после рестарта — в новый контейнер)
...
# 3. ClickHouse миграции (ПОСЛЕ рестарта — в новый контейнер с обновлённым кодом)
#    docker cp копирует файлы миграций в контейнер, затем скрипт их применяет
echo ""
echo "→ [3/6] ClickHouse migrations..."
