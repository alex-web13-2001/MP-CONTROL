...
- **CreateCampaignModal** — обработка ошибки автозапуска:
  - Новое поле `start_error` в ответе создания кампании
  - Визуальное предупреждение (amber warning card): «Кампания создана, но автозапуск не удался»
  - Кнопка «Запустить вручную» для retry через страницу управления
  - Graceful degradation: кампания сохраняется даже при провале автозапуска
- **AdvertisingAnalyticsPage** — предзагрузка баланса + рекомендации:
  - Баланс WB загружается при открытии страницы (не при открытии модала) — устраняет 30с задержку
  - `balanceLoading` state + skeleton placeholder вместо «0 ₽»
  - **Новая секция «Рекомендации по запуску рекламы»** (collapsible):
    - 3 категории товаров: продающиеся без рекламы, приостановленные кампании, стагнирующие
    - Карточки с фото товара, заказами за 30д, остатками, рекомендуемым бюджетом
    - API: `GET /advertising-analytics/ad-launch-recommendations`
