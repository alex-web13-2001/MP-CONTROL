...
Используется Redis-based dedup (`helpers._dedup_dispatch`): перед отправкой задачи ставится NX-ключ с TTL. Если ключ существует — задача уже в очереди/выполняется, повторная не ставится.

### Модульная структура задач

55 задач разбиты на 8 доменных модулей в `celery_app/tasks/`:

| Модуль | Описание |
|--------|----------|
| `coordinators.py` | Диспетчеры Beat (8 задач) |
| `wb_sync.py` | WB данные: заказы, финансы, контент (11 задач) |
| `wb_advertising.py` | WB реклама: кампании, ставки (6 задач) |
| `ozon_sync.py` | Ozon данные: товары, заказы, цены (20 задач) |
| `ozon_advertising.py` | Ozon реклама (4 задачи) |
| `onboarding.py` | Первичная загрузка магазина (2 задачи) |
