...
### fix(ozon-dashboard): Алерты «Склады» и «Продажи» — переключение на PostgreSQL

**Проблема**: Таб «Склады» в блоке «Важное» Ozon-дашборда показывал 28 товаров с stock=0 (severity=critical), хотя у всех реально были остатки (видимые на странице Цен).

**Корневая причина**: Алерты использовали ClickHouse-таблицу `fact_ozon_warehouse_stocks`, которая была **пуста** для всех магазинов. При нулевых остатках все SKU попадали в critical-алерты.

**Решение — PostgreSQL dim_ozon_products:**
- **Склады**: Остатки теперь берутся из `COALESCE(stocks_fbo, 0) + COALESCE(stocks_fbs, 0)` — тот же источник, что и страница Цен
- **Продажи**: Алерт «нет продаж 7+ дней» тоже переключён на PostgreSQL для данных об остатках
- ClickHouse по-прежнему используется для avg daily sales (14д) и last sale date
- Финансовые алерты проверены — логика корректна, без изменений

**Результат:**
- Было: 28 фейковых critical-алертов (все «стоки = 0»)
- Стало: 0 алертов (все 40 товаров имеют реальные остатки и активно продаются)

**Файлы:**
- `backend/app/api/v1/dashboard.py` — секции 5 (Warehouses) и 6 (Sales) переписаны
...
**Проблема**: Прибыль на Дашборде (-243₽) и в Товарах (+395₽) отличалась на 638₽ (ровно ad_spend).
**Причина**: Товары с рекламой, но без заказов в периоде, имели `gross_profit = None` и пропускались в суммировании итого. Дашборд при этом вычитал ВЕСЬ ad_spend (638₽) целиком.

**Исправлено:**
- `wb_products.py`: если товар имеет `ad_spend > 0`, но `orders_7d = 0` → `gross_profit = -ad_spend` (чистый убыток)
- Теперь итого прибыль в Товарах совпадает с Дашбордом

**Документация:**
- `docs/architecture/04_BACKEND_API.md` — обновлены секции Dashboard WB и Products WB:
  - Новая формула прибыли (unit economics × orders_clean − ad_spend)
  - Маркетинговая воронка (gross → net) для обоих модулей
  - Товары без заказов с рекламой = -ad_spend в итого
...
**Редизайн первого столбца таблицы кампаний** на обеих страницах рекламы (Управление + Аналитика):

**Трёхстрочная иерархия:**
- **Строка 1**: Название кампании (жирное, 14px)
- **Строка 2**: Цветные pill-бейджи (bid type + payment type) + плейсменты текстом
- **Строка 3**: ID + кол-во артикулов + статус Ozon (приглушённый)

**Цветовая система бейджей:**
- 🟠 **Ручная ставка** — amber `bg-amber-500/15`
- 🔵 **Единая** — blue `bg-blue-500/15`
- 🟣 **CPM** — violet `bg-violet-500/20`
- 🔵 **CPC** — cyan `bg-cyan-500/20`

**Файлы:**
- `frontend/src/pages/AdManagementPage.tsx` — badge column + cleanup
- `frontend/src/pages/AdvertisingAnalyticsPage.tsx` — идентичный дизайн
...
**Проблема**: В модальнике кампании «Выр. общая» (product_revenue) была меньше «Выр. рекл.» (ad_revenue). Причина — `product_revenue` использовала `finished_price` с фильтром `is_cancel = 0` (только выкупленные заказы), а WB «Выр. рекл.» считается по всем поступившим заказам.

**Решение**: Заменили `sum(finished_price) ... AND is_cancel = 0` на `sum(price_with_disc)` (все поступившие заказы, без фильтра отмен). Исправлено в обоих местах: KPI-агрегат + ежедневный time-series.

**Файлы**: `backend/app/api/v1/campaign_details.py`

---

## 2026-04-03 (v17.33.2)

### fix(events): Ложные события «Товар удалён» (ITEM_REMOVE)

**Проблема**: EventDetector генерировал тысячи ложных ITEM_REMOVE событий (4204 шт.). Товары, которые никто не удалял, отображались как «Товар удалён» в ленте событий и аналитике кампаний.

**Корневая причина**: WB API периодически возвращает пустой `nm_settings[]` для кампании во время «API-штормов». EventDetector сравнивал `old_items` (из Redis) с пустым `current_items_set`, считал все товары «удалёнными» и записывал ITEM_REMOVE в event_log. При каждом цикле Celery эта ситуация повторялась.

**Решение** — ITEM_REMOVE debouncing (аналог существующего debouncing для BID_CHANGE):

- Если API вернул 0 товаров, а в Redis были товары → пропуск ITEM_REMOVE (API glitch)
- Если >50% товаров «исчезли» за один цикл → пропуск (массовое удаление нереалистично)
- Защита добавлена в обе версии: `detect_changes()` (V1) и `detect_changes_v2()` (V2)
- Redis state НЕ перезаписывается пустым списком (`items=None` if empty) — уже было

**Очистка данных**: Удалены все 4204 ложных ITEM_REMOVE из event_log.

**Файлы**: `backend/app/services/event_detector.py`
...
**Проблема**: Период статистики кластеров был захардкожен (14 дней), без возможности переключения.

**Решение:**

**Frontend (CampaignManagementModal):**
- Новый селектор периода: кнопки `7д | 14д | 30д` с иконкой 📅 в панели табов
- По умолчанию: 14 дней (как было)
- Переключение мгновенно перезагружает данные кластеров
- Активная кнопка подсвечена фиолетовым (consistent с дизайн-системой)

**Backend (Celery sync_normquery_data):**
- Окно сбора увеличено с 3 до 7 дней (`range(3)` → `range(7)`)
- Данные за 7д полностью из кеша, 14д/30д — накапливаются со временем

**Файлы:**
- `frontend/src/components/CampaignManagementModal.tsx` — period selector UI + stateful dateRange
- `backend/celery_app/tasks/tasks.py` — sync window 3→7 days
