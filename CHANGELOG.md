...
- **Backend / `proxy_provider.py` [MODIFY]:** Auto-fallback на direct при мёртвых прокси
  - Прокси с `failure_count ≥ 10` и `success_rate = 0` считаются мёртвыми и пропускаются
  - Если ВСЕ прокси мёртвые → fallback на direct + WARNING в логах
  - Предотвращает бесконечный цикл: карантин 5 мин → retry → fail → карантин
